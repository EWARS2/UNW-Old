...
# Further changes by Ethan Reed 11/22/2024

"""
Program #1
Consider the Neural Tic Tac Toe program.
Make various changes to create a new model
(for example change the HIDDEN_SIZE,
or number of TRAINING_ITERATIONS,
or update sample4.txt -- the training data set).
Then run the program a few times.
Take screenshots showing how well it plays.
"""
...
def make_move(board, player):
    ...
    flat = [board[0][0], board[0][1], board[0][2], board[1][0], board[1][1], board[1][2], board[2][0], board[2][1],
            board[2][2]]
...
def train_model():
    ...
    with open('sample4.txt', 'r') as myFile:  # Default file is sample4.txt
        for j in range(50):
            print(j)
            for i in range(3):
...
BOARD_SIZE = 3  # Default: 3
HIDDEN_SIZE = 30  # Default: 15
TRAINING_ITERATIONS = 2000  # Default: 1000
